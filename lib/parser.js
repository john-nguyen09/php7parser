...
(function (Parser) {
    ...
    function expression(minPrecedence) {
        ...
        while (true) {
            ...
            if (binaryPhraseType === 100) {
                p.children.push(p.right = typeDesignator(101));
            }
            else {
    ...
    function methodDeclarationHeader(memberModifers) {
        ...
        if (isParameterStart(peek())) {
            p.children.push(p.parameterList = delimitedList(130, parameterDeclaration, isParameterStart, 93, [121]));
        }
        expect(121);
    ...
    function interfaceDeclaration() {
        ...
        p.children.push(p.body = typeDeclarationBody(104, isClassMemberStart, interfaceMemberDeclarations));
        return end();
    }
    ...
    function traitDeclaration() {
        ...
        p.children.push(p.body = typeDeclarationBody(166, isClassMemberStart, traitMemberDeclarations));
        return end();
    }
    ...
    function echoIntrinsic() {
        ...
        next();
        p.children.push(p.exprList = delimitedList(72, expressionInitial, isExpressionStart, 93));
        expect(88);
        return end();
    }
    ...
    function functionStaticDeclaration() {
        ...
        next();
        p.children.push(p.variableDeclarationList = delimitedList(159, staticVariableDeclaration, isStaticVariableDclarationStart, 93, [88]));
        expect(88);
        return end();
    }
    function globalDeclaration() {
        ...
        next();
        p.children.push(p.variableNameList = delimitedList(177, simpleVariable, isSimpleVariableStart, 93, [88]));
        expect(88);
        return end();
    }
    ...
    function forStatement() {
        ...
        expect(118);
        if (isExpressionStart(peek())) {
            p.children.push(p.initialiser = forExpressionGroup(82, [88]));
        }
        expect(88);
        if (isExpressionStart(peek())) {
            p.children.push(p.control = forExpressionGroup(75, [88]));
        }
        expect(88);
        if (isExpressionStart(peek())) {
            p.children.push(p.end = forExpressionGroup(80, [121]));
        }
        expect(121);
        let t = peek();
        if (t.tokenType === 87) {
    ...
    function propertyDeclaration(p) {
        let t;
        p.phraseType = 137;
        p.children.push(p.propertyList = delimitedList(139, propertyElement, isPropertyElementStart, 93, [88]));
        expect(88);
        return end();
    }
    ...
    function variableAtom() {
        ...
        switch (t.tokenType) {
            ...
            case 147:
                return qualifiedName();
            default:
                let p = start({ phraseType: 68, children: [] });
                error();
                return end();
        }
    }
    ...
    function haltCompilerStatement() {
        let p = start({ phraseType: 93, children: [] });
        next();
        expect(118);
        expect(121);
        expect(88);
    ...
    function namespaceUseDeclaration() {
        ...
        if (t.tokenType === 147 || t.tokenType === 116) {
            ...
            expect(147);
            expect(116);
            p.children.push(p.list = delimitedList(126, namespaceUseGroupClause, isNamespaceUseGroupClauseStartToken, 93, [119]));
            expect(119);
            expect(88);
            return end();
        }
        p.children.push(p.list = delimitedList(123, namespaceUseClauseFunction(nsNameNode), isQualifiedNameStart, 93, [88], true));
        expect(88);
        return end();
    }
